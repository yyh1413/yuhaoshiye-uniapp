<template>
  <view :style="{ height: addUnit(height) }" style="background: #fff;">
    <view :class="`wd-navbar ${customClass} ${fixed ? 'is-fixed' : ''} ${bordered ? 'is-border' : ''}`" :style="rootStyle" style="background-color: transparent;">
      <view class="wd-navbar__content">
        ...
        <view :class="`wd-navbar__left ${leftDisabled ? 'is-disabled' : ''}`" @click="handleClickLeft" v-else-if="!$slots.left">
          <wd-icon v-if="leftArrow" name="arrow-left" custom-class="wd-navbar__arrow" :style="{color: props.color}" />
        ...
        <view class="wd-navbar__title"  :style="{color: props.color}">
          <slot name="title" />
